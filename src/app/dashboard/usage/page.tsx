...
export default function UsagePage() {
  ...
  const { stats, overview, successRate, byProvider, byUser, recentLogs, credits, isLoading } = useUsage(days);
    ...
    <div className="space-y-6">
      ...
      {isLoading && !stats ? (
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <StatCard
            title="积分消费"
            value={(credits?.total || 0).toLocaleString()}
            subtitle={`${credits?.transactions || 0} 笔交易`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent noPadding>
            {isLoading ? (
              <div className="p-6"><SkeletonTable rows={4} cols={2} /></div>
            ) : (
              <div className="divide-y divide-gray-100">
                {(credits?.byProvider || []).map((item) => {
                  const maxCredits = Math.max(...(credits?.byProvider || []).map(p => p.credits));
        ...
        <Card>
          ...
          <CardContent noPadding>
            {isLoading ? (
              ...
              <div className="divide-y divide-gray-100">
                {(credits?.byModel || []).slice(0, 10).map((item) => {
                  const maxCredits = Math.max(...(credits?.byModel || []).map(m => m.credits));
                  const percentage = maxCredits > 0 ? (item.credits / maxCredits) * 100 : 0;
                  return (
